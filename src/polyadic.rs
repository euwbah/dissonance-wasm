...
pub fn graph_dissonance(
    ...
    for cand_idx in 0..num_candidates {
        ...
        //
        // This method requires calculating dissonances before recursive tonicity, relying on the
...
pub struct TonicityHeuristic {
    ...
    /// `tonicity_map` is a lookup of dyad tonicities of the existing notes in `cents`. The
    /// key is a bitstring of two 1s, where the power of 2 is the index of each note in the dyad in
    /// `cents`.
    ///
